...
export default function PackageCard({ pkg, index }: PackageCardProps) {
  return (
    <motion.div
      ...
      className={`relative bg-white rounded-[20px] overflow-hidden shadow-premium hover:shadow-premium-lg transition-shadow duration-300 border ${
        pkg.popular ? "border-green-200/80 ring-1 ring-green-500/10" : "border-gray-100/60"
      }`}
    >
      {pkg.popular && (
      ...
      {/* Content */}
      <div className="p-4">
        <h3 className="text-[14px] font-bold text-gray-900 leading-snug">{pkg.name}</h3>
        <p className="mt-1 text-gray-400 text-[11px] line-clamp-2">{pkg.description}</p>
        ...
        <div className="mt-2.5 space-y-1.5">
          ...
          {pkg.includes.length > 4 && <p className="text-[8px] text-green-600 font-medium pl-5">+{pkg.includes.length - 4} more</p>}
        </div>

        <div className="mt-3.5 flex items-center justify-between pt-3 border-t border-gray-100/80">
          <div>
            <span className="text-[18px] font-bold text-gray-900">₹{pkg.price.toLocaleString()}</span>
            <span className="text-[9px] text-gray-400 block mt-0.5">/person</span>
          </div>
          <button className="px-4 py-2 gradient-green text-white font-semibold rounded-xl text-[11px] shadow-sm shadow-green-700/10 hover:shadow-md transition-shadow active:scale-[0.97]">
            Book
          </button>
        </div>
      </div>
